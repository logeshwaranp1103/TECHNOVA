...
export default function StudentLayout() {
  ...
  const fetchBadgeCounts = useCallback(async () => {
    ...
    try {
      const [bookings, waitlists] = await Promise.all([
        ...
        db.read('seatsync_waitlists').catch(() => []),
      ]);
  ...
  // ─────────────────────────────────
  // MOBILE DRAWER: Escape + body scroll lock
  // ─────────────────────────────────
  useEffect(() => {
    ...
    >
      ...
      <div className={`h-16 flex items-center border-b border-slate-100 shrink-0 ${isCollapsed ? 'justify-center px-3' : 'justify-between px-4'}`}>
        <button
          ...
          className={`flex items-center gap-2.5 rounded-xl transition-all duration-150 hover:opacity-80 focus-visible:ring-2 focus-visible:ring-brandBlue/50 focus-visible:outline-none group ${isCollapsed ? '' : ''}`}
        >
          ...
          {!isCollapsed && (
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[9px] font-bold uppercase tracking-widest text-brandBlue mt-0.5">Library Portal</span>
            </div>
          )}
        </button>
        ...
        {isCollapsed && (
          <div className="absolute top-4 right-0 translate-x-full z-10">
            {/* handled below via outer button */}
          </div>
        )}
      </div>
  ...
  return (
    <div className="flex h-screen bg-slate-50/60 overflow-hidden font-sans">
      ...
      <aside className={`hidden md:flex flex-col flex-shrink-0 relative z-20 h-full transition-all duration-300 ${isSidebarCollapsed ? 'w-[76px]' : 'w-[272px]'}`}>
        {/* Expand button when collapsed (sits outside the sidebar) */}
        {isSidebarCollapsed && (
          <button
            type="button"
            onClick={toggleSidebar}
